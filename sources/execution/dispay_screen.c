...
/*   Updated: 2022/07/12 20:35:15 by lorispuchol      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_night(t_game *g)
{
	...
	sky.img = mlx_xpm_file_to_image(g->mlx_ptr, "./sprites/dark_sky.xpm", &sky.width, &sky.height);
	sky.addr = mlx_get_data_addr(sky.img, &sky.b_p_pix, &sky.l_len, &sky.endian);
	offset_dir = (g->player->dir * sky.width)  / (M_PI * 2);
	xy[1] = -1;
	while (++xy[1] < g->w_he * 0.5 + (int)(sinf(g->player->tilt) * g->w_he * 0.5) - 1)
	{
		...
		while (++xy[0] < g->w_wi)
		{
			color[0] = ((float)xy[0] / (float)g->w_wi) * sky.width + offset_dir;
			if (color[0] > sky.width)
				color[0] -=  sky.width;
			ft_set_pix(g->screen, xy[0], xy[1], ft_get_color(&sky, color[0], xy[1]));
		}
	}
	back.tl.x = 0;
