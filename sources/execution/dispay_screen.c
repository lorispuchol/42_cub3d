...
/*   Updated: 2022/07/16 04:00:39 by kmammeri         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_floor(t_game *g)
{
	...
	while (xy[1] >= g->w_he * 0.5 + (int)(sinf(g->player->tilt) * g->w_he * 0.5))
	{
		...
		color[1] = (int)(((float)i / (float)y_2) * g->graph->ground->height) + sinf(g->player->tilt) * g->graph->ground->height;
		...
		while (xy[0] < g->w_wi)
		{
			color[0] = (int)(((float)(xy[0] - (g->w_wi - x_len) * 0.5) / (float)x_len) * g->graph->ground->width);
