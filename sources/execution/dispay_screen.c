...
/*   Updated: 2022/07/20 22:30:28 by kmammeri         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_floor(t_game *g)
{
	...
	offset[0] = (g->player->x - floorf(g->player->x));
	offset[1] = (g->player->y - floorf(g->player->y));
	// offset[0] = cosf(g->player->dir) * g->graph->ground->width;
	// offset[1] = sinf(g->player->dir) * g->graph->ground->height;
	xy[1] = g->w_he - 1;
	y_2 = g->w_he * 0.5 - (int)(sinf(g->player->tilt) * g->w_he * 0.5);
	x_len = g->w_wi - g->w_wi * sinf(g->player->tilt) * 0.5;
	coef_dir = (float)(x_len * 0.5) / (float)(g->w_he * 0.5 - (int)(sinf(g->player->tilt) * g->w_he * 0.5));
	y_2 = y_2 * 0.5;
	i = 0;
	while (xy[1] >= g->w_he * 0.5 + (int)(sinf(g->player->tilt) * g->w_he * 0.5))
	{
		...
		color[1] = (int)(((float)(i + offset[1] * 10) / (float)y_2) * g->graph->ground->height);
		...
		while (xy[0] < g->w_wi)
		{
			color[0] = (int)(((float)(xy[0] - (g->w_wi - x_len) * 0.5) / (float)x_len) * g->graph->ground->width) + (sinf(g->player->dir - g->fov)) *g->graph->ground->width;
			while (color[0] >= g->graph->ground->width)
				color[0] -=  g->graph->ground->width;
			while (color[0] < 0)
				color[0] +=  g->graph->ground->width;
			ft_set_pix(g->screen, xy[0], xy[1], ft_get_color(g->graph->ground, color[0], color[1]));
			xy[0]++;
		}
		x_len = x_len - coef_dir * 2;
		if (x_len <= 0)
			x_len = 1;
		xy[1]--;
		i++;
